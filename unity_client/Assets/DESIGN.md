...
**解決策: ローカルCORSプロキシ（`build/server.ps1`）**

`start-server.bat` から起動されるPowerShellサーバーに、`/proxy/`エンドポイントとして
...
- リクエストボディ転送
- SSEストリーミング中継（`SendChunked = true` + バッファ逐次書き込み + `Flush()`）
...
**実装上の注意点:**
- `RawUrl`を使用（`Url.AbsolutePath`は`//`を`/`に正規化してしまうため）

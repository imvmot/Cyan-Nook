...
**起動時のターゲット初期化（重要）:**
LookAtClipは「視線追従の重み（weight）」を時間軸で制御するだけで、注視ターゲット
（`_currentTargetType` と `playerTarget`）は別途セットされている必要がある。
`CharacterSetup.lookAtCameraOnLoad` を **true** にしておくと、起動時に
`LookAtPlayer()` が呼ばれて `_currentTargetType=Player` + `playerTarget=MainCamera` が
セットされ、entry/walk等のLookAtClipが有効化された時点でカメラ方向を注視する。
false の場合 `_currentTargetType=None` のままなので、TalkController等が明示的に
ターゲットを設定するまで LookAtClip を置いても視覚上は動かない。
...
|-------|----------------|
| `LLMClient` | LLM 通信統合管理、ILLMProvider 切替、ブロッキング/ストリーミング両方式対応、`AbortRequest()` でリクエスト中断、フィールド逐次イベント転送、`GetCorsProxyUrl()`静的ヘルパーでClaude/OpenAI接続をローカルCORSプロキシ経由に自動変換 |
...
| F | 家具インタラクション開始 | 最寄りの家具あり、インタラクション中でないこと（exit/entry専用家具は除外） |
...
画像パラメータは `List<string>` で複数画像に対応。null/空 = 画像なし、1要素 = キャラクターカメラのみ、2要素 = キャラクターカメラ＋共有画面。各プロバイダーはリスト内の全画像をループしてAPI固有フォーマットに変換する。
...
ブラウザのCORSポリシーにより、WebGLビルドからの直接APIアクセスはプロバイダー側のCORS設定に依存:

| プロバイダー | WebGL直接接続 | 理由 |
|------------|--------------|------|
| Gemini | ✅ 動作 | GoogleがCORSポリシーでブラウザリクエストを許可 |
| Ollama / LMStudio | ✅ 動作 | localhost（同一オリジン） |
| Dify | ✅ 動作 | セルフホストでCORS設定可能 |
| **Claude** | ❌ ブロック | Anthropic APIがブラウザリクエストを許可していない |
| **OpenAI** | ❌ ブロック | OpenAI APIがブラウザクライアントを明示的にブロック |

**解決策: ローカルCORSプロキシ（`build/server.ps1` / `build/server.js`）**

起動用スクリプトから立ち上がるローカルサーバーに、`/proxy/`エンドポイントとして
リバースプロキシ機能を組み込み。`http://localhost:PORT`から配信されたWebGLビルドの場合、
`LLMClient.GetCorsProxyUrl()`がClaude/OpenAIのリクエストURLを自動的にプロキシ経由に変換する。

| OS | 起動スクリプト | サーバー実装 |
|----|--------------|-------------|
| Windows | `start-server.bat` | `server.ps1`（PowerShell HttpListener） |
| macOS / Linux | `start-server.sh` | `server.js`（Node.js 18+、組み込みモジュールのみ） |
| Windows（任意） | `start-server-node.bat` | `server.js`（Node.js版のテスト・代替用） |

両サーバーは同一のエンドポイント仕様（`/proxy/<encoded-url>`、同じ許可ヘッダー、同じCORSヘッダー）
を提供するため、Unity側は切り替え不要で両方に対応する。

```
ブラウザ(WebGL) → http://localhost:8080/proxy/{エンコード済URL}
                  → server.ps1がCORSヘッダー付与＋転送
                  → https://api.anthropic.com/v1/messages
```

**プロキシの機能:**
- OPTIONSプリフライトに`Access-Control-Allow-Origin: *`等で応答
...
**Unity側の自動切替（`LLMClient.GetCorsProxyUrl()`）:**
- WebGLビルドかつ`Application.absoluteURL`が`http://localhost`で始まる場合のみプロキシ経由
- エディタ実行時、tailscale等の非localhost配信時は直接接続
- ClaudeProvider/OpenAIProviderのSend/Streaming/TestConnectionの3箇所で適用

**実装上の注意点:**
- PowerShell版は`RawUrl`を使用（`Url.AbsolutePath`は`//`を`/`に正規化してしまうため）
- Node.js版は`req.url`から`/proxy/`以降を直接切り出し（`url.parse`は使わない）
- OpenAI gpt-4o以降は`max_tokens`ではなく`max_completion_tokens`を使用する必要あり（変更済）
- Node.js版は組み込みモジュールのみで実装し、`npm install`不要
- Node.js版のブラウザ自動起動（`spawn`）は`'error'`イベントを必ずハンドルする。Linuxの最小構成やDockerコンテナ等で`xdg-open`が存在しない場合、未ハンドルだと`ENOENT`でプロセスごとクラッシュするため
...
画面共有（Screen Capture）がアクティブな場合は、キャラクターカメラ画像に加えて共有画面の画像も同時に送信する（最大2枚）。
これにより、キャラクターがユーザーの方を向いている状態でも共有画面の内容をLLMが認識できる。
...
適用箇所: `CaptureVisionImages()` ヘルパーメソッド（`SendChatMessage()` / `SendAutoRequest()` から呼び出し）。共有画面の画像も同時に抑制される。
...
全プロバイダーが `List<string> imagesBase64` で複数画像を受け取り、ループで各画像をAPI固有フォーマットに変換する。

**Ollama:**
- `images` 配列に複数のbase64文字列を格納

```json
{
  "model": "llava",
  "prompt": "...",
  "images": ["camera_base64", "screen_base64"]
}
```

**OpenAI / LM Studio:**
- content配列内に複数の `image_url` オブジェクト
...
```json
{
  ...
  "content": [
    {"type": "text", "text": "..."},
    {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64,camera_base64"}},
    {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64,screen_base64"}}
  ]
}
```

**Claude:**
- content配列内に複数の `image` オブジェクト（`source.type: "base64"`）

**Gemini:**
- parts配列内に複数の `inline_data` オブジェクト（`mime_type: "image/jpeg"`）

**Dify:**
- 各画像を順次 `/files/upload` でアップロードし、取得した複数の `upload_file_id` を `files` 配列で参照
- 個別画像のアップロード失敗時はスキップして残りを続行（グレースフルデグラデーション）

```
① POST /files/upload × N枚
   → Response: { "id": "file_id_1" }, { "id": "file_id_2" }

② POST /chat-messages
   → Body: { ..., "files": [
       {"type":"image","transfer_method":"local_file","upload_file_id":"file_id_1"},
       {"type":"image","transfer_method":"local_file","upload_file_id":"file_id_2"}
     ] }
```
...
#### 画像収集フロー（ChatManager.CaptureVisionImages）

```
ChatManager.SendMessage() / SendAutoRequest()
  ↓ useVision=true && !IsVisionSuppressed
CaptureVisionImages()
  ├─ CharacterCameraController.CaptureImageAsBase64()  → images[0]
  └─ ScreenCaptureDisplayController.CaptureImageAsBase64()  → images[1]（IsPlaying時のみ）
  ↓ List<string> imagesBase64
LLMClient.SendRequest/SendStreamingRequest(imagesBase64)
  ↓
ILLMProvider 実装がAPI固有フォーマットに変換
```
...
ユーザーのデスクトップ画面（またはウィンドウ）を3Dシーン内のQuadに表示する。
さらに、共有画面がアクティブ時はキャラクターカメラ画像とは別に共有画面の画像もLLMに直接送信される（Vision System参照）。

PC: ブラウザの Screen Capture API（`getDisplayMedia()`）を使用。
モバイル: Screen Capture APIが非対応のため、背面カメラ（`WebCamTexture`）に自動フォールバック。
`Application.isMobilePlatform`で判定し、同一コンポーネント内でモードを切り替える。
...
| メソッド | 説明 |
|---------|------|
| `StartCapture()` | PC: ブラウザの画面共有ダイアログを表示。モバイル: 背面カメラを起動 |
| `StopCapture()` | キャプチャ/カメラを停止、Rendererを無効化 |
| `IsPlaying` (property) | キャプチャ/カメラが動作中かどうか |
| `IsMobileMode` (property) | モバイルデバイスかどうか（背面カメラモード） |
| `CaptureImageAsBase64(int jpegQuality = 75)` | 現在のフレームをJPEGエンコードしてbase64文字列を返す |
| `GetTexture()` | キャプチャ用Texture2Dを返す（UIプレビュー用） |
...
#### jslib プラグイン（MobileCamera.jslib）

モバイル背面カメラ検索用。`enumerateDevices()`でカメラ一覧を取得し、ラベルから背面カメラを特定する。

| 関数 | 説明 |
|------|------|
| `MobileCamera_FindRearCamera()` | 背面カメラの非同期検索を開始。権限未付与時は`getUserMedia(facingMode: ideal: environment)`で権限取得後に再列挙 |
| `MobileCamera_IsReady()` | 検索結果が準備できたか（0=pending, 1=ready） |
| `MobileCamera_GetLabel()` | 検索結果のデバイスラベルを返す（UTF8文字列、`_malloc`確保。空文字=見つからず） |

**背面カメラ検索ロジック:**
1. `enumerateDevices()`でvideoinputデバイスを列挙
2. ラベルが空（権限未付与）→ `getUserMedia(facingMode: ideal: environment)`で権限取得 → 再列挙
3. ラベルに "back"/"rear"/"背面"/"environment" を含むデバイスを優先
4. 見つからなければ "front"/"前面"/"user"/"selfie" を含まないデバイスを選択

**処理フロー（PC、ScreenCapture.jslib）:**
```
...
- **モバイル自動フォールバック**: `Application.isMobilePlatform`で検出し、背面カメラ（`WebCamTexture`）に切り替え。
  ...
  プレビューはWebCamTextureを直接RawImageに割り当て（自動更新、Y軸反転不要）

#### Y軸反転補正

ブラウザCanvas（`getImageData()`）のY座標系はUnity Texture2Dと逆のため、2箇所で補正が必要:

| 用途 | PC（ScreenCapture） | モバイル（WebCamTexture） |
|------|---------------------|------------------------|
| 3Dシーン表示（Quad） | `mainTextureScale = (1, -1)` + `offset = (0, 1)` | `mainTextureScale = (1, 1)`（反転不要） |
| LLM送信画像 | ピクセル行の上下入れ替え後にJPEGエンコード | `GetPixels()`→直接JPEGエンコード（反転不要） |
| UIプレビュー（RawImage） | `uvRect = (0, 1, 1, -1)` | `uvRect = (0, 0, 1, 1)`（反転不要） |
...
4. ChatManager の `screenCaptureController` に同じ Quad をドラッグ＆ドロップ
5. LLM設定パネルに RawImage を追加し、`LLMSettingsPanel.screenCapturePreviewImage` にアサイン
6. LLM設定パネルの Screen Capture トグルで ON/OFF 可能
7. カメラプレビュートグルOFF時は共有画面プレビューも連動して非表示になる
...
**Entry再生中のレスポンスキューイング:**
Entry再生中にLLM応答が到着した場合、CharacterControllerでentryタイムラインを上書きしないよう
逐次反映を抑制した上で、ChatManager側でレスポンスをキューに保持し、
Entry完了時に発火する。これにより Entry Prompt の応答 `action=move/interact/emote/message` を
キャラクターが entry 完了直後に正常に実行する。

```
LLM応答到着（IsPlayingEntry == true）
├─ CharacterController:
│   ├─ HandleStreamField → 破棄してフラグ立て（_streamFieldsDroppedDuringEntry）
│   │   └─ Entry完了後の後続フィールドも逐次反映せずブロック処理に委ねる（straddle対策）
│   ├─ HandleChatResponse → IsPlayingEntryガードはセーフネットとして残置
│   └─ HandleChatStateChanged → Thinking 状態遷移をスキップ
└─ ChatManager.HandleChatResponse:
    └─ _entryQueuedResponse にキュー保持
        └─ Entry完了時にOutingController.OnEntryAnimationCompletedイベント発火
            └─ ChatManager.FlushEntryQueuedResponse → OnChatResponseReceived
                └─ CharacterController.HandleChatResponse がブロック処理で全フィールド適用
```

**Entry早期完了（ActionCancelClip連携）:**
Entry Timelineに `ActionCancelClip` が配置されていると、ChatManagerがレスポンスを
キューした時点で`OutingController.RequestEarlyEntryComplete()` を呼び出し、
`CharacterAnimationController.RequestCancelAtRegion()` でCancelRegion到達時に
...
`PlayState(skipBlend: true)` は `HasInertialBlendTrack()` の呼び出しをスキップする。
...
| `ScreenCaptureDisplayController` | PC: ユーザーのデスクトップ/ウィンドウ画面をシーン内Quadに表示（Screen Capture API）。モバイル: 背面カメラにフォールバック |
...
6. LateUpdate [ExecutionOrder 20000] - InertialBlendHelper
   ...
   └─ PrePass無しの場合（フォールバック）:
      ...
      └─ 状態に応じた処理（WaitingFirstFrame/Blending）
...
InertialBlendClip.targetBones (Inspector上でAdvancedDropdown / テンプレートボタンから設定)
  ↓
[Runtime: PlayState() / PlayTimeline()]
HasInertialBlendTrack() でIBトラック存在確認 + 古いIBをCancelBlend()
  ↓
director.Evaluate() → PlayableGraph評価
  ↓
[InertialBlendMixerBehaviour.ProcessFrame()]
クリップ位置に到達時、InertialBlendHelper.StartInertialBlend(duration, targetBones) を呼び出し
  ↓
...
| メソッド/プロパティ | 用途 |
|---|---|
| `StartInertialBlend(duration, targetBones)` | InertialBlendMixerBehaviourから呼び出し（指定ボーン） |
...
- WaitingFirstFrame: 突然のポーズ変化で髪・揺れ物がポップ
...
| IBステート | PrePassの処理 |
|---|---|
| WaitingFirstFrame | クリーン保存→前ポーズ適用（SpringBoneが突然の変化を見ない） |
...
2. Update()（毎フレーム）
   ...
   ※ このチェックは _hasLoopRegion ガードの前に置かれており、
     LoopRegion を持たないTimeline（entry01等）でも有効。
...
#### キャンセル実行フロー（OutingController連携・Entry早期完了）

Entry Timelineに `ActionCancelClip` を配置し、Entry中にLLMレスポンスが
ChatManagerにキューされた場合、CancelRegion到達時点でEntryを早期完了させる。

```
1. ChatManager.HandleChatResponse（Entry中にレスポンス到着）
   ├→ _entryQueuedResponse にキュー保持
   └→ outingController.RequestEarlyEntryComplete()
       ├→ HasCancelRegions == false なら return（従来通り InteractionEndClip まで再生）
       ├→ animationController.OnCancelRegionReached を購読
       └→ animationController.RequestCancelAtRegion()

2. Update()（CancelRegion到達フレーム）
   └→ FireCancelRegionReached → OnCancelRegionReached イベント発火

3. OutingController.OnCancelRegionReached_HandleEarlyComplete:
   ├→ 購読解除
   └→ OnEntryAnimationComplete を早期実行
       ├→ ドア家具停止 / NavMeshAgent再有効化 / per-callコールバック発火
       └→ OnEntryAnimationCompleted イベント発火
           └→ ChatManager.FlushEntryQueuedResponse → CharacterControllerに dispatch
```
...
| `Scripts/Character/InteractionController.cs` | インタラクション ed中のキャンセルハンドリング・直接アクション実行 |
| `Scripts/Character/OutingController.cs` | Entry早期完了（RequestEarlyEntryComplete） |
...
Thinking / Emote タイムライン（talk_thinking01 / emote_happy01 等）の途中に配置し、
「配置フレームでタイムラインを完全打ち切り、復帰先ステートに即時ループ再開、
同フレームからIBで補間する」を発火するカスタムトラック。
通常タイムラインにおける「アクションキャンセル＋補間」のThinking/Emote版。
加算実行中 / 非加算実行中の両方で動作する（加算時はポーズ連続性のための追加処理が走る）。
...
3. **下半身スナップによる揺れ物ポップ**: 上半身42ボーンのみをIB対象にしていた初期実装では、
   直前のthink_ed入りIBを `CancelBlend` する瞬間に非対象の下半身10ボーンが一時的に
   thinkingクリーンポーズへスナップし、Hips/Spineを親に持つ髪SpringBoneが跳ねていた。
4. **補間元ポーズずれ**: `SnapshotCurrentPoseAsClean` を AO 復元 LateUpdate 前に実行すると
   下半身も「thinking全身ポーズ」で記録され、IBが「立ちthink → sit」に見える補間になる。
5. **Hips沈み込みオーバーシュート**: `_prevCleanPose`（N-2フレームのクリーンポーズ）に
   AO 復元前の全身thinkingポーズが残存すると、v₀ = lastClean - prevClean が偽の下向き速度
   として検出され、IB5次多項式がHipsを下方向に外挿して沈み込みが発生する。
...
└── AdditiveCancelTrack
    └── AdditiveCancelClip
        ...
        │    （空欄時は全Humanoidボーン52本を自動選択 — Eye/Jaw 除外、
        │      下半身スナップによる揺れ物ポップ対策）
        └── クリップの長さ = IB補間時間
```
...
3. bones 決定: クリップの `overrideBones` があればそれを使用、なければ `GetAllHumanoidBones()`
   （Eye/Jaw 除外、全52ボーン）
4. `ForceCompleteAdditiveTimelineWithBlend(duration, bones)` を呼ぶ
...
2. **AO が Active なら**次の3ステップでIB補間元ポーズを正しく確定（加算中のみ）:
   - `AdditiveOverrideHelper.ApplyRestoreNow()` — LateUpdateを待たずに復元処理を手動実行し、
     transformを「見えていたポーズ（下半身スナップショット＋上半身加算）」に揃える
   - `InertialBlendHelper.SnapshotCurrentPoseAsClean()` — 上記ポーズを `_lastCleanPose`
     （IBのN-1補間元）として確定
   - `InertialBlendHelper.InvalidatePrevCleanPose()` — `_prevCleanPose`（N-2）を無効化し
     v₀=0 フォールバックを強制（AO復元前の「thinking全身」が残存して偽の下向き速度を
     発生させる問題を防ぐ）
3. `AdditiveOverrideHelper.StopOverride(invalidateCleanPose: false)` — 加算解除するが
   IBのprev clean poseは保持（AO非Active時はno-op）
...
6. `InertialBlendHelper.StartInertialBlend(duration, bones)` で全身のIBを強制開始
   （既存のthink_ed入りIBはCancelBlendで上書き）

**非加算Thinking** の場合: 上記2のAOブロックは全てスキップされ、3のStopOverrideもno-op。
通常のIB pipelineが維持している `_lastCleanPose` をそのまま補間元として使用する。
4〜6は同一なので「PlayState(復帰先) + 全身IB」だけが走る。

Emote中の場合: `_emoteReturnState` / `OnEmoteEndPhaseComplete` / `OnEmoteTimelineStopped`
の解除を行い、同じ3ステップ（ApplyRestoreNow→Snapshot→InvalidatePrev）＋復帰＋IB。

それ以外で AO が Active の場合: 保険として同じ3ステップ + AO 停止 + IB のみ。
...
| 配置先Timeline | 配置位置 | 効果 |
|---|---|---|
| TL_talk_thinking01 | think_ed 開始フレーム | think_ed本体を再生せず即復帰＋全身IB |
| TL_emote_happy01 | emote_ed 後半 | emote_edの最後までは再生せず途中で切上＋全身IB |

#### ChatManager との連携: emote到着時のthinking解除経路統一

[ChatManager.HandleThinkingExitOnField](../unity_client/Assets/Scripts/Chat/ChatManager.cs)
は過去、`emote` フィールド到着時のみ `ForceStopThinking`（即時キャンセル）を呼んでいた。
これだと think_ed 上に配置した AdditiveCancelClip が評価される前に thinking director が
停止するため、加算解除時の補間が一切機能せず sit 復帰時にポーズがスナップしていた。

**修正後**: `emote / emotion / target / reaction` の全てで graceful な `StopThinking` を呼ぶ
ように統一。think_ed が自然再生され、配置された AdditiveCancelClip が必ず発火する。
emote本体の再生は CharacterController の `FlushAfterThinkExit`（Think Exit Delay）で
`_isThinkingActive=false` 後に遅延実行される。

これにより **AdditiveCancelClip がthinking/emote解除の唯一の発火経路** となり、
アーティストが配置した Clip のduration・位置・overrideBones が常に尊重される。

#### 既存パスへの影響

- 通常のstandalone再生時（AO非Active）も Mixer は正常動作（非加算Thinkingの補間として機能）
- 既存の `StopOverride()`（4箇所）はデフォルト引数で従来動作
- AdditiveCancelClipが配置されていないThinking/Emote Timelineは従来通り
  `OnThinkingEndPhaseComplete` / `OnEmoteEndPhaseComplete` 経路で正常復帰する
  （emote/actionは Think Exit Delay 経由で適切に遅延される）

#### 関連ファイル

| ファイル | 役割 |
|---|---|
| `Scripts/Timeline/AdditiveCancelTrack.cs` | TrackAsset + MixerBehaviour、`GetAllHumanoidBones()` ヘルパー |
| `Scripts/Timeline/AdditiveCancelClip.cs` | PlayableAsset（overrideBones フィールド） |
| `Scripts/Timeline/AdditiveOverrideHelper.cs` | `CurrentAdditiveBones` / `StopOverride(bool)` / `ApplyRestoreNow()` |
| `Scripts/Character/CharacterAnimationController.cs` | `ForceCompleteAdditiveTimelineWithBlend()` / AdditiveCancelTrackのBind |
| `Scripts/Chat/ChatManager.cs` | `HandleThinkingExitOnField` で emote/emotion/target/reaction を統一的に graceful stop |
...
// → skipBlend: trueの場合、InertialBlendTrackの検出・IBキャンセルをスキップ。VRM初回読み込み時のT-pose補間回避に使用

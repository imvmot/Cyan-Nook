...
- パース失敗 → 不完全JSON修復を試みる（`RepairIncompleteJson`）
  ...
  - 修復失敗 → `OnParseError(errorMessage, rawText)` を発火（UI表示用）
...
| common_entry01 | OneShot | ドアから入る（入室、Root Motion移動） |
...
| `FirstRunController` | 初回起動ポップアップUI + WebLLMモデルダウンロード進捗表示（RoomLightController経由でライト制御） |
...
| `StatusOverlay` | ステータスオーバーレイ（FPS・キャラクターステート・再生中タイムライン・JSヒープメモリを常時表示）。DebugSettingsPanel のトグルで表示切替 |
...
[Sleep中の動作]
  ...
  ├─ RoomLightController でライト消灯（リアルタイムLight + Emission + Lightmap OFF）
  └─ アプリ終了 → 再起動時もsleepループから再開（startOffで起動時から消灯）

[起床時]
  └─ OnWakeUpAnimationComplete() で RoomLightController.SetLightsOn()（全て復元）
```
...
| `roomLightController` | RoomLightController | - | ライトON/OFF連動（睡眠時消灯・起床時点灯） |
...
| メソッド | 説明 |
|---------|------|
| `EnterSleep(int durationMinutes, string furnitureId)` | 睡眠状態に遷移。タイマー設定、PlayerPrefs保存、IdleChat停止、Boredom停止、ライト消灯 |
...
│       └─ 現在時刻 >= sleep_wake_time → sleep_state OFF → 通常起動（common_entry01再生）
```
...
- 入室はLLMが `common_entry01` アニメーションを選定した場合のみ（自律判断）
...
| `entryAnimationId` | string | "common_entry01" | 入室アニメーションID |
...
| OutingController | common_entry01のEntry完了検知（PlayState後に動的購読） |

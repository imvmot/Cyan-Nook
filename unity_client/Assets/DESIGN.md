...
| `CaptureImageAsBase64(int jpegQuality = 75)` | 現在のフレームをJPEGエンコードしてbase64文字列を返す。Y軸反転補正済み |
...
| 用途 | 補正方法 |
|------|----------|
| 3Dシーン表示（Quad） | Material の `mainTextureScale = (1, -1)` + `mainTextureOffset = (0, 1)` |
| LLM送信画像（`CaptureImageAsBase64`） | ピクセル行の上下入れ替え後にJPEGエンコード |
| UIプレビュー（RawImage） | `uvRect = (0, 1, 1, -1)` でUV反転 |
